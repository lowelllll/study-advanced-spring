...
class HelloTraceV2 {
    ...
    fun beginSync(beforeTraceId: TraceId, message: String): TraceStatus {
        val status = TraceStatus(beforeTraceId.createNextId(), System.currentTimeMillis(), message)
        logger.info { "[${status.traceId.id}] ${addSpace(START_PREFIX, status.traceId.level)} $message " }
        return status
    }
